...
export class MessageLegacyService implements IMessageLegacyService {
  ...
  private async loadMessages(sessionId: string): Promise<Message[]> {
    ...
    return entries.map((msg, index) => {
      // v1 parity: `created_at` uses the wire record time when known, nudged
      // to stay strictly increasing so cursor consumers keep a stable total
      // order; fall back to the session creation time plus the index.
