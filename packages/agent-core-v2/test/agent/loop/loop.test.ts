...
describe('Agent loop', () => {
  ...
  it('restores the engine turn clock from the machine journal on resume', async () => {
    ...
    expect(records.some((record) => record.type === 'human.agent.turn.ended')).toBe(true);
    ...
    try {
      ...
      expect(
        persisted
          .filter((record) => record.type === 'human.agent.turn.ended')
  ...
  `);
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [wire] human.agent.message.appended        { "message": { "message": { "role": "tool", "content": [ { "type": "text", "text": "lookup-result" } ], "toolCallId": "call_lookup" }, "meta": { "source": "tool" } }, "time": "<time>", "kind": "event" }
