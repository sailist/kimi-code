...
Plugins 也可以在 manifest 中声明 MCP servers。Plugin 声明的 servers 默认启用：在 `/plugins` 中禁用会立即生效——进行中的会话里工具仍然可见，但调用会失败并返回移除提示——重新启用则需要 `/reload` 或新会话。详见 [Plugins](./plugins.md#plugin-中的-mcp-servers)。
